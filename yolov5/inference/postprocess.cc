...
static int process_i8(int8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      ...
                      int32_t zp, float scale)
{
    int validCount = 0; // 记录当前输出层最终保留多少个候选框
    int grid_len = grid_h * grid_w;
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    int validCount = 0; // 三个输出层合计保留下来的候选框数量
    int stride = 0; //保存当前输出层相对于输入图像的步长。
    int grid_h = 0; // 保存当前输出特征图的网格高度和宽度。
    int grid_w = 0;
    int model_in_w = app_ctx->model_width; // 从上下文中取模型输入尺寸。
